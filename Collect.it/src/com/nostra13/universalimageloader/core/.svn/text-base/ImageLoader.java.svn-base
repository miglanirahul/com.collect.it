...
public class ImageLoader {
	...
	/**
	 * Initializes ImageLoader's singleton instance with configuration. Method
	 * should be called <b>once</b> (each following call will have no effect)<br />
	 * 
	 * @param configuration
	 ...
	 *            process. Listener fires events on UI thread.
	 * 
	 * @throws IllegalStateException
	 *             if {@link #init(ImageLoaderConfiguration)} method wasn't
	 *             called before
	 ...
	 *             called before
	 */
	public void displayImageWithBitmap(Bitmap bitmap, ImageView imageView,
			DisplayImageOptions options, ImageLoadingListener listener) {
		if (configuration == null) {
		...
		ImageSize targetSize = getImageSizeScaleTo(imageView);
		/*
		 * String memoryCacheKey = MemoryCacheKeyUtil.generateKey(uri,
		 * targetSize); cacheKeyForImageView.put(imageView, memoryCacheKey);
		 */

		if (bitmap != null && !bitmap.isRecycled()) {
			if (configuration.loggingEnabled)// Log.i(TAG,
												// String.format(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE,
												// memoryCacheKey));
				listener.onLoadingStarted();
			Bitmap displayedBitmap = options.getDisplayer().display(bitmap,
					imageView);
			...
			/*
			 * ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri,
			 * imageView, targetSize, options, listener, getLockForUri(uri));
			 * LoadAndDisplayImageTask displayImageTask = new
			 * LoadAndDisplayImageTask(configuration, imageLoadingInfo, new
			 * Handler()); boolean isImageCachedOnDisc =
			 * configuration.discCache.get(uri).exists(); if
			 * (isImageCachedOnDisc) {
			 * cachedImageLoadingExecutor.submit(displayImageTask); } else {
			 * imageLoadingExecutor.submit(displayImageTask); }
			 */
		}
	}
	...
	private void checkExecutors() {
		if (imageLoadingExecutor == null || imageLoadingExecutor.isShutdown()) {
			imageLoadingExecutor = Executors.newFixedThreadPool(
		...
		if (cachedImageLoadingExecutor == null
			...
			cachedImageLoadingExecutor = Executors.newFixedThreadPool(
					configuration.threadPoolSize,
					configuration.displayImageThreadFactory);
		}
	}
	 ...
	 * 
	 * @param context
	 *            Application context (used for creation of fake
	 *            {@link ImageView})
	 * @param uri
	 *            Image URI (i.e. "http://site.com/image.png",
	 *            "file:///mnt/sdcard/image.png")
	 ...
	 *            {@linkplain ImageLoadingListener#onLoadingComplete(Bitmap)
	 ...
	 *             called before
	 */
	public void loadImage(Context context, String uri, ImageSize minImageSize,
			ImageLoadingListener listener) {
	 ...
	 *            Incoming options should contain {@link FakeBitmapDisplayer} as
	 *            displayer.
	 * @param listener
	 *            {@linkplain ImageLoadingListener Listener} for image loading
	 *            process. Listener fires events on UI thread.
	...
	public void loadImage(Context context, String uri, ImageSize minImageSize,
		...
		if (options.getDisplayer() instanceof FakeBitmapDisplayer) {
			...
			optionsWithFakeDisplayer = new DisplayImageOptions.Builder()
					.cloneFrom(options).displayer(fakeBitmapDisplayer).build();
		}

		ImageView fakeImage = new ImageView(context);
		fakeImage.setLayoutParams(new LayoutParams(minImageSize.getWidth(),
				minImageSize.getHeight()));
	...
	private ExecutorService createExecutor() {
		...
		return new ThreadPoolExecutor(configuration.threadPoolSize,
				configuration.threadPoolSize, 0L, TimeUnit.MILLISECONDS,
				taskQueue, configuration.displayImageThreadFactory);
	}
	 ...
	 * 3) Get <b>maxImageWidthForMemoryCache</b> and
	 * <b>maxImageHeightForMemoryCache</b> from configuration. If both of them
	 * are not set then go to step #3.<br />
