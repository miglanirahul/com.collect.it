...
import com.collect.it.adapter.TagsHorizontalViewAdapter.ViewHolder;
...
public class TagGalleryEditAdapter extends BaseAdapter {
	 ...
	 */
	public TagGalleryEditAdapter(Activity context,
	...
	public int getCount() {
		...
		if (CollectItSharedDataModel.getInstance().getTagGalleryEditArrayList() != null) {
			return CollectItSharedDataModel.getInstance()
					.getTagGalleryEditArrayList().size();
		} else {
			return 0;
		}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			if (CollectItSharedDataModel.getInstance()
					.getTagGalleryEditArrayList().size() > position) {
				holder.textView.setText(CollectItSharedDataModel.getInstance()
						.getTagGalleryEditArrayList().get(position)
						.getTagTitle());
				holder.textView.setBackgroundResource(CollectItSharedDataModel
						.getInstance().getTagGalleryEditArrayList()
						.get(position).getTagImageBackground());
				/*try {
				...
				public void onClick(View v) {
					if (onImageGalleryItemClickListener != null) {
						onImageGalleryItemClickListener.onItemSelected(v,
								position, ViewPagerIds.TAG_GALLERY_ID.name());
					}
				}
			});
